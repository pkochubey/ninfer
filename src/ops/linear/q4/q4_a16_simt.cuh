...
__global__ __launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void q4_a16_simt_kernel(
...
#pragma unroll
    for (int prefetch = 0; prefetch < S - 1; ++prefetch) {
        if (prefetch < stages) {
            q4_simt_issue_stage<Schedule, FullK>(stage_codes[warp][prefetch],
...
#pragma unroll 1
    for (int stage = 0; stage < stages; ++stage) {
        ...
        if (fetch < stages) {
            q4_simt_issue_stage<Schedule, FullK>(stage_codes[warp][fetch % S],
        ...
        cp_wait<S - 1>();
        __syncwarp();
        q4_simt_consume_stage<Schedule, FullK, Full>(
