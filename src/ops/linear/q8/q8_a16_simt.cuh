...
__device__ __forceinline__ void q8_simt_issue_stage(uint4* __restrict__ shared_codes,
    ...
    const std::uint8_t* stage_codes  = code_row + group0 * Q8RowSplitStorage::kCodeBytesPerGroup;
    const std::uint8_t* stage_scales = scale_row + group0 * Q8RowSplitStorage::kScaleBytesPerGroup;

    const int active_code_vecs = FullStage ? kCodeVecs
                                           : active_groups * Q8RowSplitStorage::kCodeBytesPerGroup /
                                                 static_cast<int>(sizeof(uint4));
    for (int vec = lane; vec < kCodeVecs; vec += 32) {
        if (FullStage || vec < active_code_vecs) {
            q8_simt_copy_code<Schedule>(&shared_codes[vec],
...
q8_simt_consume_stage(const __nv_bfloat16* __restrict__ x, std::int32_t k, int col0,
...
#pragma unroll
    for (int phase = 0; phase < kCodePhases; ++phase) {
        const int group        = phase * 8 + (lane >> 2);
        const int stage_groups = FullStage ? Schedule::kGroupsPerWarpStage : active_groups;
        if (group < stage_groups) {
            const uint2 packed = reinterpret_cast<const uint2*>(shared_codes)[phase * 32 + lane];
            const unsigned scale_pair = shared_scales[group >> 1];
            const float scale = __half2float(__ushort_as_half(scale_pair >> ((group & 1) * 16)));
            float weights[8];
#pragma unroll
            for (int i = 0; i < 8; ++i)
                weights[i] = static_cast<float>(static_cast<std::int8_t>(
                                 (i < 4 ? packed.x : packed.y) >> ((i & 3) * 8))) *
                             scale;
...
__global__ __launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void q8_a16_simt_kernel(
    const __nv_bfloat16* __restrict__ x, const std::uint8_t* __restrict__ codes,
    const std::uint8_t* __restrict__ scales, Output output, Epilogue epilogue, int rows, int k,
    int tokens, int padded_k, int token_begin) {
    ...
    int group_begin         = 0;
    const int vector_groups = k % 8 == 0 ? (k / 64) * 2 : 0;
    ...
    const std::int64_t group0 = static_cast<std::int64_t>(safe_row) * (padded_k / 32) + group_begin;
    const auto* code_row      = codes + group0 * 32;
    const auto* scale_row     = scales + group0 * 2;
    const auto* warp_x        = x + group_begin * 32;
...
#pragma unroll
    for (int prefetch = 0; prefetch < S - 1; ++prefetch) {
        if (prefetch < stages) {
            q8_simt_issue_stage<Schedule, FullK>(stage_codes[warp][prefetch],
                                                 stage_scales[warp][prefetch], code_row, scale_row,
                                                 prefetch, min(G, groups - prefetch * G), lane);
        } else {
            cp_commit();
        }
    }
#pragma unroll 1
    for (int stage = 0; stage < stages; ++stage) {
        ...
        if (fetch < stages) {
            q8_simt_issue_stage<Schedule, FullK>(stage_codes[warp][fetch % S],
                                                 stage_scales[warp][fetch % S], code_row, scale_row,
                                                 fetch, min(G, groups - fetch * G), lane);
        } else {
            cp_commit();
        }
        cp_wait<S - 1>();
        __syncwarp();
        q8_simt_consume_stage<Schedule, FullK, Full>(
            warp_x, k, token0, active_tokens, stage, min(G, groups - stage * G),
            stage_codes[warp][stage % S], stage_scales[warp][stage % S], lane, acc);
        __syncwarp();
    }
    // Warp zero owns the remainder; other K warps only own complete scale pairs.
    if (!FullK && split == 0 && row < rows) {
        for (int group = vector_groups; group < (k + 31) / 32; ++group) {
            const int kk = group * 32 + lane * 2;
            if (lane < 16 && kk < k) {
                float w0, w1;
                Q8ScalarDecodeAtom::load_pair(codes, nullptr, scales,
                                              std::int64_t(row) * (padded_k / 32) + group, lane, w0,
                                              w1);
#pragma unroll
