...
#include "ops/linear/q5/q5_a16_mma.cuh"

namespace ninfer::ops::detail {

template <class Schedule, class Output, class Epilogue = LinearIdentityEpilogue>
void launch_q5_a16_mma(const Q5LinearOperands& operands, Output output, Epilogue epilogue,
                       cudaStream_t stream) {
    ...
    if (operands.padded_k % Schedule::kBlockK != 0) {
        throw std::invalid_argument("Q5 MMA padded K must contain complete K tiles");
    }
    for_each_token_slice(operands.tokens, Schedule::kBlockTokens, [&](int offset, int count) {
        ...
        if (full) {
            q5_a16_mma_kernel<Schedule, true><<<grid, Schedule::kThreads, 0, stream>>>(
                x, operands.codes, operands.high, operands.scales, output, epilogue, operands.rows,
                operands.k, count, operands.padded_k, offset);
        } else {
            q5_a16_mma_kernel<Schedule, false><<<grid, Schedule::kThreads, 0, stream>>>(
                x, operands.codes, operands.high, operands.scales, output, epilogue, operands.rows,
                operands.k, count, operands.padded_k, offset);
        }
