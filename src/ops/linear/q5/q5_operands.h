...
inline Q5LinearOperands q5_linear_operands(const Tensor& x, const Weight& weight) {
...
// Private template domain. Public shape admission remains in q5_dispatch.
inline void validate_q5_operands(const Q5LinearOperands& operands) {
    ...
    if (operands.rows <= 0 || operands.tokens <= 0 || operands.k <= 0 ||
        ...
        throw std::invalid_argument(
            "Q5 templates require positive N/K/T and padded K aligned to 128");
    }
    if (!aligned(operands.x, 16) || !aligned(operands.codes, 16) || !aligned(operands.high, 16) ||
        !aligned(operands.scales, 4)) {
        throw std::invalid_argument("Q5 templates require aligned activation and weight planes");
    }
}
